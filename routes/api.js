...
router.get('/wishlist', requireAuth, (req, res) => {
  ...
  res.json([
    { name: 'Kerala', image: 'https://images.unsplash.com/photo-1590050752117-238cb0fb12b1?w=400' },
    { name: 'Manali', image: 'https://images.unsplash.com/photo-1621617249658-f7541ef42a93?w=400' }
  ]);
});
  ...
  const tips = [
    {
      ...
      content: 'October to March is generally the best time to visit most parts of India.',
      category: 'Planning'
    },
    {
      id: 2,
      title: 'Packing Essentials',
      content: 'Pack light, comfortable clothes and always carry a first-aid kit.',
      category: 'Packing'
    },
    {
      id: 3,
      title: 'Local Transportation',
      content: 'Use local trains and buses for authentic experiences and budget travel.',
      category: 'Transport'
    },
    {
      id: 4,
      title: 'Food Safety',
      content: 'Drink bottled water and eat at busy local restaurants for fresh food.',
      category: 'Health'
    }
  ];
...
router.get('/weather/:city', (req, res) => {
  ...
  const weather = {
    ...
    forecast: [
      { day: 'Today', temp: Math.floor(Math.random() * 20) + 20, condition: 'Sunny' },
      { day: 'Tomorrow', temp: Math.floor(Math.random() * 20) + 20, condition: 'Cloudy' },
      { day: 'Day 3', temp: Math.floor(Math.random() * 20) + 20, condition: 'Clear' }
    ]
  };
...
router.post('/budget', requireAuth, (req, res) => {
  const { destination, days, accommodation, transport, food, activities } = req.body;
  ...
  budget.total = Object.values(budget.breakdown).reduce((sum, cost) => sum + cost, 0);
